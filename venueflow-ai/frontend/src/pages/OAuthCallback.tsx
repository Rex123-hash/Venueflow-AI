import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuthStore } from '../stores/authStore';
import toast from 'react-hot-toast';

/**
 * Handles the redirect from Google OAuth.
 * Backend redirects to: /auth/callback?token=JWT&user=JSON
 * or on failure:        /auth/callback?error=message
 */
const OAuthCallback: React.FC = () => {
  const navigate = useNavigate();
  const { login } = useAuthStore();
  const [status, setStatus] = useState<'loading' | 'error'>('loading');
  const [errorMsg, setErrorMsg] = useState('');
  const hasProcessed = React.useRef(false);

  useEffect(() => {
    if (hasProcessed.current) return;
    hasProcessed.current = true;

    let timeoutId: NodeJS.Timeout;
    const params = new URLSearchParams(window.location.search);
    const token = params.get('token');
    const userRaw = params.get('user');
    const error = params.get('error');

    if (error) {
      setErrorMsg(decodeURIComponent(error));
      setStatus('error');
      toast.error('Google login failed. Please try again.');
      timeoutId = setTimeout(() => navigate('/login'), 3000);
      return () => clearTimeout(timeoutId);
    }

    if (!token || !userRaw) {
      setErrorMsg('Incomplete response from Google. Please try again.');
      setStatus('error');
      timeoutId = setTimeout(() => navigate('/login'), 3000);
      return () => clearTimeout(timeoutId);
    }

    try {
      const user = JSON.parse(decodeURIComponent(userRaw));
      login(user, token);
      toast.success(`Welcome, ${user.name}! 🎉`);

      // Redirect based on role
      switch (user.role) {
        case 'MANAGER':
        case 'ADMIN':
          navigate('/manager/dashboard');
          break;
        case 'STAFF':
          navigate('/staff/view');
          break;
        default:
          navigate('/fan/dashboard');
      }
    } catch {
      setErrorMsg('Failed to process login. Please try again.');
      setStatus('error');
      timeoutId = setTimeout(() => navigate('/login'), 3000);
    }

    return () => {
      if (timeoutId) clearTimeout(timeoutId);
    };
  }, [login, navigate]);

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-bg-primary">
      {status === 'loading' ? (
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-6 relative">
            <div className="w-16 h-16 rounded-full border-4 border-accent/20 border-t-accent animate-spin" />
            {/* Google G inside spinner */}
            <svg className="absolute inset-0 m-auto w-7 h-7" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
          </div>
          <h2 className="text-xl font-bold mb-2">Signing you in with Google...</h2>
          <p className="text-text-secondary text-sm">Just a moment</p>
        </div>
      ) : (
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-danger/20 flex items-center justify-center">
            <svg className="w-8 h-8 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <h2 className="text-xl font-bold mb-2 text-danger">Login Failed</h2>
          <p className="text-text-secondary text-sm mb-4">{errorMsg}</p>
          <p className="text-text-secondary text-xs">Redirecting back to login...</p>
        </div>
      )}
    </div>
  );
};

export default OAuthCallback;
